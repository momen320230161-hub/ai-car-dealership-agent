...
def google_login():
    ...
    # Default allowed redirect URLs in Supabase Auth dashboard:
...
def callback():
    ...
    # Protect against session fixation by regenerating session state
    session.clear()
    login_user(user, remember=True)
    next_page = request.args.get("next")
    if not next_page or not next_page.startswith("/"):
        next_page = url_for("chat.chat_page")
